...
'use strict';
const tracks=[
 {title:'505',artist:'Arctic Monkeys',src:'505.mp3',art:'archive/night-collage.jpg',note:'This one gets special treatment. Obviously.',mood:'come-back-to-me energy'},
 ...
 {title:'Y-3',artist:'Elias OS',src:'Y-3.mp3',art:'mirror pic.PNG',note:'A tiny artifact from before the playlist became a whole problem.',mood:'archive track'}
];
const K='elias.music.';
...
function load(autoplay=false){const t=current();if(!audio.src.endsWith(encodeURI(t.src))&&!audio.src.endsWith(t.src.replace(/ /g,'%20'))){audio.src=t.src;audio.currentTime=Number(localStorage.getItem(K+'time.'+idx)||0)||0}save();renderMini();if($('#musicPlayerPage'))renderPlayer();if(autoplay)audio.play().catch(()=>{})}
...
function renderMini(){injectMini();const m=$('#eliasMiniPlayer'),t=current();$('img',m).src=t.art;$('strong',m).textContent=t.title;$('b',m).textContent=audio.paused?'▶':'Ⅱ';m.classList.toggle('playing',!audio.paused)}
...
function openMusic(){if(window.homeScreen)homeScreen.classList.add('hidden');if(window.statusBar)statusBar.classList.add('hidden');if(window.appWindow)appWindow.classList.remove('hidden');renderPlayer();window.scrollTo(0,0)}
function renderPlayer(){if(!window.appContent||!window.appTitle)return;appTitle.textContent='Music';const t=current();appContent.innerHTML=`<div id="musicPlayerPage" class="music-player-page"><section class="music-cover"><img src="${t.art}" alt=""><div class="music-cover-grad"></div><span>OURS</span></section><section class="music-meta"><small>${t.artist}</small><h3>${t.title}</h3><p>${t.note}</p><em>${t.mood}</em></section><section class="music-progress-wrap"><input id="musicProgress" type="range" min="0" value="0"><div><span id="musicNow">0:00</span><span id="musicDur">0:00</span></div></section><section class="music-main-controls"><button id="musicPrev">‹‹</button><button id="musicPlay" data-music-play>▶</button><button id="musicNext">››</button></section><section class="music-sub-controls"><button id="musicShuffle" class="${shuffle?'on':''}">↝ Shuffle</button><button id="musicRepeat" class="${repeat!=='off'?'on':''}">↻ ${repeat==='one'?'One':repeat==='all'?'All':'Repeat'}</button></section><section class="music-queue"><small>OUR PLAYLIST</small>${tracks.map((x,i)=>`<button data-track="${i}" class="${i===idx?'active':''}"><img src="${x.art}"><span><strong>${x.title}</strong><em>${x.artist}</em></span><b>${i===idx&&!audio.paused?'♪':''}</b></button>`).join('')}</section></div>`;$('#musicPlay').onclick=toggle;$('#musicPrev').onclick=prev;$('#musicNext').onclick=next;$('#musicShuffle').onclick=toggleShuffle;$('#musicRepeat').onclick=cycleRepeat;$('#musicProgress').oninput=e=>{audio.currentTime=Number(e.target.value)||0};appContent.querySelectorAll('[data-track]').forEach(b=>b.onclick=()=>{idx=Number(b.dataset.track);load(true)});updateProgress();updatePlayButtons()}
function hijackExistingMusic(){document.querySelectorAll('[data-app="music"]').forEach(b=>{b.addEventListener('click',e=>{e.stopImmediatePropagation();e.preventDefault();openMusic()},true)});const old=window.toggleMusic;window.toggleMusic=async()=>{toggle();return !audio.paused};}
window.openMusic=openMusic;window.eliasTracks=tracks;injectMini();renderMini();hijackExistingMusic();load(false);
})();